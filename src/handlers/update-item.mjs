...
export const handler = async (event) => {
  ...
  try {
    ...
    if (!hasUpdate) {
      return badRequest("At least one field must be provided for update", updatableFields);
    }
    ...
    logger.error("UpdateItem failed", { requestId, itemId, error: err.message, stack: err.stack });
    return serverError();
  }
};
